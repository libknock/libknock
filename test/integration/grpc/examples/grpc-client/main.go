...
func main() {
	...
	defer cancel()
	conn, err := grpc.DialContext(ctx, "passthrough:///"+addr, grpc.WithContextDialer(func(ctx context.Context, address string) (net.Conn, error) { return d.DialContext(ctx, "tcp", address) }), grpc.WithTransportCredentials(credentials.NewTLS(&tls.Config{ServerName: "localhost", InsecureSkipVerify: true})), grpc.WithBlock())
	if err != nil {
		log.Fatal(err)
	}
	defer conn.Close()
