...
`TTLLRU.Len()` is a stored-entry upper bound. It can include expired entries until a read path or `Sweep` removes them, so dashboards should not treat it as an exact active-entry count. Use `ActiveLen(now)` for a non-mutating active count and `LenAfterSweep(now)` when the caller intentionally wants to remove expired entries first. For pressure signals, prefer sweep-aware active counts where available and record replay-cache-full or limiter-full decisions separately. The Prometheus adapter exports `libknock_auth_replay_cache_full_total`, `libknock_auth_replay_cache_len`, and `libknock_auth_replay_cache_capacity` when the auth path reports `ErrReplayCacheFull`; treat that as either abusive fake-nonce traffic or an undersized cache.

Relay pending-queue pressure is reported through `RelayErrorEvent{Stage:"pending_full", DroppedCount, Pending}` and exported as `libknock_relay_pending_full_total`, `libknock_relay_dropped_total`, and `libknock_relay_pending_current`. `netx.AuthenticatedListener` reports the same backpressure class through `OnAuthDrop(..., netx.ErrAuthBackpressure, pending)`; the Prometheus adapter normalizes it to `auth_backpressure` and increments the same dropped/pending counters. Spikes usually mean a burst; sustained growth means the auth worker count, upstream latency, or caller-side rate limits need attention.
