...
重放缓存、敲门会话存储、封禁列表和限流器有相关的存储机制，但具有不同的安全语义。`TTLLRU.Len()` 报告存储的条目，可能包括尚未扫描的已过期条目；报告容量压力时请使用活动计数或扫描感知指标。重放缓存、限流器和封禁列表在容量耗尽时都失败关闭。封禁列表会保留活动封禁，而不会按 LRU 将其驱逐；需要获知容量状态的调用方可使用 `BanE`/`BanUntilE` 并处理 `ErrBanListFull`。脚本防火墙后端会在 `Init()` 中验证配置的命令，而不会执行 allow/revoke/cleanup 脚本。
